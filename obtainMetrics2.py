def obtainMetrics2(classifier, x, y, sensitive, objectiveWeights = zeros(5,1)):
    ...
    tp_non_protected = 1 - sum(positiveClassification(sensitive)==0 & positive(sensitive)==1)/sum(positive(sensitive)==1)
    tp_protected = 1 - sum(positiveClassification(nonSensitive)==0 & positive(nonSensitive)==1)/sum(positive(nonSensitive)==1)
    
    tn_non_protected = 1 - sum(correctClassifcation(sensitive)==0 & positive(sensitive)==0)/sum(positive(sensitive)==0)
    tn_protected = 1 - sum(correctClassifcation(nonSensitive)==0 & positive(nonSensitive)==0)/sum(positive(nonSensitive)==0)
    
    tp_predicted_count = sum(positiveClassification(sensitive)==1 & positive(sensitive)==1) + sum(positiveClassification(nonSensitive)==1 & positive(nonSensitive)==1)
    averall_positive = sum(positive(sensitive)==1) + sum(positive(nonSensitive)==1)
    
    tn_predicted_count =sum(correctClassifcation(sensitive)==1 & positive(sensitive)==0) + sum(correctClassifcation(nonSensitive)==1 & positive(nonSensitive)==0)
